...
extern "C" void* pmem_alloc(size_t size, int *pfd)
{
    int         fd;
    size_t      aligned_size;
    void*       base;
...
extern "C" void* pmem_alloc_sync(size_t size, int *pfd)
{
    int         fd;
    size_t      aligned_size;
    void*       base;
...
extern "C" int  pmem_free(void *ptr, size_t size, int fd)
{
    int err, ret = 0;
...
extern "C" void* pmem_get_phys(int fd)
{
...
extern "C" void pmem_cache_flush(int fd, unsigned int offset, unsigned int length) {
    pmem_region region = { 0, 0 };
    int         err = -1; 
...
void pmem_cache_clean(int fd, unsigned int offset, unsigned int length) {
    ...
    if (fd < 0 || offset == 0 || length == 0) {
        ALOGE("pmem_cache_clean: invalide argument\n");
        return;
    }   
    ...
    if (IOCTL_FAILED == err)
    {   
        ALOGE("PMEM_CACHE_CLEAN offset 0x%08x, size %d failed!\n", offset, length);
    }
}
...
void pmem_cache_inv(int fd, unsigned int offset, unsigned int length) {
    ...
    if (fd < 0 || offset == 0 || length == 0) {
        ALOGE("pmem_cache_inv: invalide argument\n");
        return;
    }   
    ...
    if (IOCTL_FAILED == err)
    {   
        ALOGE("PMEM_CACHE_INV offset 0x%08x, size %d failed!\n", offset, length);
    }
}
#endif
